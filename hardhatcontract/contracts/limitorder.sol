...
contract TideDexLimitOrder is Ownable {
    ...
    function setNewWETH(address weth) external onlyOwner {
        WETH = weth;
    }
        ...
        payable // Must attach ETH equal to the `value` field from the API response.
    {
        ...
        if(orderType == OrderType.EthForTokens){
            (bool success,) = swapTarget.call{value: assetInOffered}(swapCallData);
            require(success, 'SWAP_CALL_FAILED');
