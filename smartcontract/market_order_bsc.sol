...
contract PancakeRouter is IPancakeRouter02 {
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint orderId = ordersNum;
        ordersNum++;
        orderBook[orderId] = Order(msg.sender,path[0],path[path.length-1],amounts[0],amountOut,orderId, orders.length);
        ordersForAddress[msg.sender].push(orderId);
        orders.push(orderId);
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint orderId = ordersNum;
        ordersNum++;
        orderBook[orderId] = Order(msg.sender,path[0],path[path.length-1],amountIn,amounts[amounts.length - 1],orderId, orders.length);
        ordersForAddress[msg.sender].push(orderId);
        orders.push(orderId);
        ...
        returns (uint[] memory amounts)
    {
        ...
        IWETH(WETH).deposit{value: amounts[0].add(stakeValue)}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
    ...
    function myswapExactETHForTokens(uint amountIn,uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        payable
        ensure(deadline)
        returns (uint[] memory amounts)
    {
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        uint stakeValue = amountIn.mul(STAKE_FEE).div(1000);
        ...
        //IWETH(WETH).withdraw(stakeValue);

        uint orderId = ordersNum;
        ordersNum++;
        orderBook[orderId] = Order(msg.sender,path[0],path[path.length-1],amounts[0],amountOutMin,orderId, orders.length);
        ordersForAddress[msg.sender].push(orderId);
        orders.push(orderId);
